...
SELECT CARTON_ID, (C.LEN*C.WIDTH*C.HEIGHT) AS CARTON_VOL FROM CARTON AS C
	WHERE (C.LEN*C.WIDTH*C.HEIGHT) > 
    (SELECT SUM(P.LEN*P.WIDTH*P.HEIGHT*OI.PRODUCT_QUANTITY) AS PRODUCT_VOL 
	 FROM PRODUCT AS P 
...
SELECT OC.CUSTOMER_ID, (CONCAT(OC.CUSTOMER_FNAME,' ',OC.CUSTOMER_LNAME)) AS CUSTOMER_FULLNAME, OH.ORDER_ID, SUM(OI.PRODUCT_QUANTITY) AS PRODUCT_QUANT
...
#------------------------------------------------------------------------------------------------------------------------------------
# 9. Write a query to display the order_id, customer id and cutomer full name of customers along with (product_quantity) 
# as total quantity of products shipped for order ids > 10060. 
# (6 ROWS) [NOTE: TABLES TO BE USED - online_customer, order_header, order_items]
#------------------------------------------------------------------------------------------------------------------------------------

SELECT OI.ORDER_ID, OC.CUSTOMER_ID, (CONCAT(OC.CUSTOMER_FNAME,' ',OC.CUSTOMER_LNAME)) AS CUSTOMER_FULLNAME, SUM(OI.PRODUCT_QUANTITY) AS TOTAL_QUANTITY
	FROM ONLINE_CUSTOMER AS OC 
			INNER JOIN 
         ORDER_HEADER AS OH
			ON OC.CUSTOMER_ID = OH.CUSTOMER_ID
	...
	GROUP BY CONCAT(OC.CUSTOMER_FNAME,' ',OC.CUSTOMER_LNAME);
...
SELECT P.PRODUCT_ID, P.PRODUCT_DESC, (OI.PRODUCT_QUANTITY) AS TOTAL_QUANTITY, (OI.PRODUCT_QUANTITY * P.PRODUCT_PRICE) AS TOTAL_VALUE, P.PRODUCT_CLASS_CODE, A.COUNTRY
FROM PRODUCT AS P INNER JOIN ORDER_ITEMS AS OI
ON P.PRODUCT_ID = OI.PRODUCT_ID
INNER JOIN ORDER_HEADER AS OH
ON OI.ORDER_ID = OH.ORDER_ID
INNER JOIN ONLINE_CUSTOMER AS OC
ON OC.CUSTOMER_ID = OH.CUSTOMER_ID
INNER JOIN ADDRESS AS A
ON A.ADDRESS_ID = OC.ADDRESS_ID
WHERE A.COUNTRY!= 'USA' AND A.COUNTRY!= 'INDIA' AND OI.PRODUCT_QUANTITY = (SELECT MAX(OI.PRODUCT_QUANTITY) FROM ORDER_ITEMS)
